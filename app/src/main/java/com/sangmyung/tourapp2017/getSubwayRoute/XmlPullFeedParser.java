package com.sangmyung.tourapp2017.getSubwayRoute;
...
public class XmlPullFeedParser extends BaseFeedParser {
    ...
    public List<SubwayRouteData> parse() {
        List<SubwayRouteData> busRouteDatas = null;
        XmlPullParser parser = Xml.newPullParser();
        try {
            parser.setInput(this.getInputStream(), null);

            int eventType = parser.getEventType();
            SubwayRouteData currentData = null;
            boolean done = false;
            while (eventType != XmlPullParser.END_DOCUMENT && !done){
                ...
                switch (eventType){
                    case XmlPullParser.START_DOCUMENT:
                        // 스트림의 시작. 리스트 생성.
                        busRouteDatas = new ArrayList<SubwayRouteData>();
                        break;
                    case XmlPullParser.START_TAG:
                        // 태그를 식별한 뒤 태그에 맞는 작업을 수행
                        name = parser.getName();
                        if (name.equalsIgnoreCase(ITEM)){
                            currentData = new SubwayRouteData();
        ...
        return busRouteDatas;//getdata list 반환
    }
}